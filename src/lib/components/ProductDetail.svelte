...
<div class="min-h-screen w-full bg-surface px-3.5 pb-16 pt-20 sm:px-6 md:pt-28">
	<a
		...
		class="mx-auto mb-4 inline-flex max-w-5xl items-center gap-1.5 text-xs font-semibold text-text-muted transition-colors hover:text-primary sm:text-sm md:mb-6"
	>
		<ArrowLeft size={14} />
		{STRINGS.PRODUCT.BACK_TO_SHOP}
	</a>

	<div class="mx-auto grid max-w-5xl grid-cols-1 items-start gap-8 lg:grid-cols-2 lg:gap-12">
		<div
			class="flex items-center justify-center rounded-2xl border border-surface-alt/70 bg-surface-alt/30 p-6 shadow-sm backdrop-blur-sm sm:p-8"
		>
			<img
				src={product.image ? product.image.replace('../', '/') : STRINGS.SHOP.FALLBACK_IMAGE}
				alt={product.title}
				class="max-h-72 rounded-lg object-contain shadow-md transition-transform duration-300 hover:scale-105 sm:max-h-96"
			/>
		</div>
		...
		<div class="flex flex-col space-y-6 sm:space-y-8">
			<div class="space-y-4 sm:space-y-5">
					...
					<div class="mt-2.5 flex items-center gap-2.5">
						<div class="flex items-center text-primary">
							...
							<span class="ml-1 text-xs text-text-muted"
								>({totalReviews} {STRINGS.PRODUCT.REVIEWS_COUNT_SUFFIX})</span
							>
						</div>
						<span class="text-xs text-text-muted"
							>• {product.sold_count || 0} {STRINGS.SHOP.SOLD}</span
						>
					</div>
				</div>
				...
				<div class="flex items-center gap-2 text-xs sm:text-sm">
					<span class="text-text-muted">{STRINGS.PRODUCT.STOCK}</span>
					{#if isOutOfStock}

<script lang="ts">
	...
	$: subdistricts =
		(locationsData as Record<string, Record<string, string[]>>)[form.city]?.[form.district] || [];
...
{#if useNewAddress}
	<AddressFieldsNewAddress bind:form {districts} {subdistricts} {inputClass} {selectClass} />
{/if}

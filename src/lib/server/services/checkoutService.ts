import { UserRepository } from '$lib/server/repositories/userRepository.js';
import { UserAddressRepository } from '$lib/server/repositories/userAddressRepository.js';
import { OrderTransactionRepository } from '$lib/server/repositories/orderTransactionRepository.js';
import { OrderRepository } from '$lib/server/repositories/orderRepository.js';
import { logger } from '$lib/server/utils/logger.js';
import {
	sendOrderConfirmationEmail,
	sendAdminNotificationEmail
} from '$lib/server/utils/mailer.js';
import { STORE_CONSTANTS, SEARCH } from '$lib/constants/config.js';
import { STRINGS } from '$lib/constants/strings.js';
import type { EmailOrderItem } from '$lib/server/utils/emailStyles.js';

function generateTrackingNumber() {
	const prefix = STORE_CONSTANTS.ORDER_PREFIX;
	const timestamp = Date.now().toString().slice(-8);
	const random = Math.floor(Math.random() * SEARCH.TRACKING_RANDOM_RANGE)
		.toString()
		.padStart(3, '0');
	return `${prefix}${timestamp}${random}`;
}

export const getAddress = async (userId: number) => {
	return await UserAddressRepository.getAddress(userId);
};

export const updateAddress = async (userId: number, address: string) => {
	await UserAddressRepository.updateAddress(userId, address);
};

interface CartItem {
	product_id: number;
	quantity: number;
	price: number;
}

async function generateMidtransToken(
	trackingNumber: string,
	total: number,
	user: Record<string, unknown> | null
) {
	const { snap } = await import('$lib/server/utils/midtrans.js');
	const transaction = await snap.createTransaction({
		transaction_details: {
			order_id: trackingNumber,
			gross_amount: Math.round(total)
		},
		customer_details: {
			first_name: user?.username || STORE_CONSTANTS.DEFAULT_CUSTOMER_NAME,
			email: user?.email || STORE_CONSTANTS.DEFAULT_CUSTOMER_EMAIL
		}
	});
	return transaction.token;
}

async function notifyAdmins(
	trackingNumber: string,
	total: number,
	username: string,
	items: EmailOrderItem[]
) {
	try {
		const adminEmails = await UserRepository.getAdminEmails();
		if (adminEmails.length > 0) {
			await sendAdminNotificationEmail(adminEmails, trackingNumber, total, username, items);
		}
	} catch (e) {
		logger.error('Failed to get admin emails:', e as Error);
	}
}

async function dispatchOrderEmails(
	user: Record<string, unknown> | null,
	total: number,
	trackingNumber: string,
	orderId: number
) {
	if (!user?.email || typeof user.email !== 'string') return;
	const username =
		typeof user.username === 'string' ? user.username : STORE_CONSTANTS.DEFAULT_CUSTOMER_NAME;
	const rawItems = await OrderRepository.fetchAllOrderItems([orderId]);
	const emailItems = rawItems.map((i) => ({
		title: (i.title as string) || STRINGS.ORDER.ITEMS,
		quantity: Number(i.quantity) || 1,
		price: Number(i.price_at_time) || 0
	}));

	await Promise.allSettled([
		sendOrderConfirmationEmail(user.email, total, trackingNumber, emailItems).catch((e) => {
			logger.error('Failed to send invoice email:', e);
		}),
		notifyAdmins(trackingNumber, total, username, emailItems)
	]);
}

export const processPayment = async (
	userId: number,
	cartItems: CartItem[],
	total: number,
	shippingAddress: string,
	shippingPrice: number,
	shippingMethod: string
) => {
	const trackingNumber = generateTrackingNumber();
	const orderId = await OrderTransactionRepository.createOrderTransaction(
		userId,
		cartItems,
		total,
		shippingAddress,
		shippingPrice,
		shippingMethod,
		trackingNumber
	);

	const user = await UserRepository.getById(userId);
	const snapToken = await generateMidtransToken(trackingNumber, total, user);
	await OrderTransactionRepository.savePaymentToken(orderId, snapToken);
	await dispatchOrderEmails(user, total, trackingNumber, orderId);

	return { orderId, trackingNumber, snapToken };
};

export const getPendingPaymentToken = async (orderId: number, userId: number) => {
	const order = (await OrderRepository.getPendingPayment(orderId, userId)) as {
		id: number;
		payment_token: string | null;
		total: number;
		tracking_number: string;
	} | null;
	if (!order) return null;
	if (order.payment_token) {
		return { orderId: order.id, snapToken: order.payment_token };
	}
	const user = await UserRepository.getById(userId);
	const snapToken = await generateMidtransToken(order.tracking_number, order.total, user);
	await OrderTransactionRepository.savePaymentToken(order.id, snapToken);
	return { orderId: order.id, snapToken };
};

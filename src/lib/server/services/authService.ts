...
import { APP_CONFIG, AUTH } from '$lib/constants/config.js';
import { AUTH_STRINGS } from '$lib/constants/authStrings.js';
...
export const loginUser = async (
	...
	if (checkVerification && user.is_verified === 0) {
		throw new AuthException(AUTH_STRINGS.MESSAGES.ACCOUNT_NOT_VERIFIED);
	}
...
export const registerUser = async (username: string, password: string, email: string) => {
	...
	if (result.isFirstUser) {
		const payload = { id: user?.id, username, email, role: AUTH.ROLE_ADMIN };
		const token = jwt.sign(payload, JWT_SECRET, { expiresIn: APP_CONFIG.JWT_EXPIRES_IN });
		return { isFirstUser: true, user, token };
	} else {
		const otp = Math.floor(AUTH.OTP_MIN + Math.random() * AUTH.OTP_RANGE).toString();
...
export const verifyUserOtp = async (userId: number, otp: string) => {
	...
	if (!isValid) throw new AuthException(AUTH_STRINGS.MESSAGES.INVALID_OTP);
...
export const processForgotPassword = async (email: string) => {
	...
	if (user) {
		const token = crypto.randomBytes(AUTH.RESET_TOKEN_BYTES).toString('hex');

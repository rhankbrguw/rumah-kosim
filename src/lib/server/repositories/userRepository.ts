...
export const UserRepository = {
	async getByUsername(username: string) {
		const sql =
			'SELECT id, username, password, email, role, avatar, is_verified FROM users WHERE LOWER(username) = LOWER(?) OR LOWER(email) = LOWER(?) LIMIT 1';
		const rows = (await db.query(sql, [username, username])) as RowDataPacket[];
		return rows[0] || null;
	},

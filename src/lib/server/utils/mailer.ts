...
import {
	...
	panelStyles,
	subTextStyles,
	renderEmailItemsTable,
	renderEmailItemsText,
	getSender,
	getAppBaseUrl,
	type EmailOrderItem
} from './emailStyles.js';
...
const send = async (to: string, subject: string, html: string, text: string) => {
	...
	try {
		const response = await fetch(
			...
			buildResendPayload(apiKey, to, subject, html, text)
		);
		if (!response.ok) {
...
export const sendOrderConfirmationEmail = async (
	...
	const itemsHtml = renderEmailItemsTable(items);
	const itemsText = renderEmailItemsText(items);
	const html = wrapEmailDocument(
		STRINGS.EMAIL.INVOICE.TITLE,
		`<h2 style="${headingStyles}">${STRINGS.EMAIL.INVOICE.TITLE}</h2><p style="${bodyTextStyles}">${STRINGS.EMAIL.INVOICE.GREETING} ${STRINGS.EMAIL.INVOICE.MESSAGE}</p><div style="${panelStyles}">${STRINGS.ORDER.TRACKING_NUMBER}: ${trackingNumber}</div>${itemsHtml}<div style="${panelStyles}">${STRINGS.EMAIL.INVOICE.TOTAL} ${formattedTotal}</div><a href="${historyLink}" style="${buttonStyles}">${STRINGS.EMAIL.INVOICE.BUTTON}</a>`
	);
	const text = `${STRINGS.EMAIL.INVOICE.GREETING}\n${STRINGS.EMAIL.INVOICE.MESSAGE}\n\n${STRINGS.ORDER.TRACKING_NUMBER}: ${trackingNumber}\n\n${itemsText}\n\n${STRINGS.EMAIL.INVOICE.TOTAL} ${formattedTotal}\n\nView your orders: ${historyLink}`;
...
export const sendAdminNotificationEmail = async (
	...
	const html = wrapEmailDocument(
		...
		`<h2 style="${headingStyles}">${STRINGS.EMAIL.ADMIN.TITLE}</h2><p style="${bodyTextStyles}">${STRINGS.EMAIL.ADMIN.MESSAGE} <strong>${username}</strong>.</p><div style="${panelStyles}">${STRINGS.ORDER.TRACKING_NUMBER}: ${trackingNumber}</div>${itemsHtml}<div style="${panelStyles}">Total: ${formattedTotal}</div><a href="${adminLink}" style="${buttonStyles}">${STRINGS.EMAIL.ADMIN.BUTTON}</a>`
	);

...
export const handleProfileUpdate = async (
	...
	if (password && password.length >= 6) {
		dataToUpdate.password = await bcrypt.hash(password, APP_CONFIG.BCRYPT_SALT_ROUNDS);
	}
	...
	return jwt.sign(payload, JWT_SECRET, { expiresIn: APP_CONFIG.JWT_EXPIRES_IN });
};

const makeResetError = (error: string) => ({
...
export const handleResetPassword = async (token: string, password: string) => {
	...
	if (!user || new Date() > new Date(user.reset_expires_at)) {
		return makeResetError(STRINGS.RESET_PASSWORD.MESSAGES.INVALID_TOKEN);
	}

	const isSamePassword = await bcrypt.compare(password, user.password);
	if (isSamePassword) {
		return makeResetError(STRINGS.RESET_PASSWORD.MESSAGES.SAME_PASSWORD);
	}

	...
	<div class="space-y-4">
		{#if userAddresses.length > 0}
			<div class="space-y-1.5">
				<label for="saved_address" class="block text-sm font-medium text-text-muted"
				...
				<select
					...
					class="w-full rounded-lg border border-surface-alt/80 bg-surface px-4 py-2.5 text-text-main focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
				>
		...
		<div class="space-y-1.5">
			<label for="address" class="block text-sm font-medium text-text-muted"
				>{STRINGS.PROFILE.FIELDS.ADDRESS}</label
			>
			<div class="relative">
				<div
					class="pointer-events-none absolute left-0 top-3 z-10 flex items-center pl-3 text-text-muted"
				>
					<MapPin size={18} />
				</div>
				<textarea
...
<div class="mb-8 mt-8 rounded-xl border border-surface-alt bg-surface/50 p-5 sm:p-6 lg:p-8">
	<h2
		class="mb-5 flex items-center gap-2 border-b border-surface-alt pb-3 text-lg font-semibold text-text-main"
	>
	...
	<div class="space-y-1.5">
		<label for="password" class="block text-sm font-medium text-text-muted"

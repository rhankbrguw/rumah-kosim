...
const reviewSchema = z.object({
	...
	comment: z
		.string()
		.min(1, STRINGS.VALIDATION.REVIEW_EMPTY)
		.max(1000, STRINGS.VALIDATION.REVIEW_TOO_LONG)
});
...
export const actions = {
	review: async ({ request, locals }: RequestEvent) => {
		if (!locals.user) {
			return fail(HTTP_STATUS.UNAUTHORIZED, { error: STRINGS.COMMON.UNAUTHORIZED });
		}

		const reviewForm = await superValidate(request, zod(reviewSchema));
		if (!reviewForm.valid) return fail(HTTP_STATUS.UNPROCESSABLE_ENTITY, { reviewForm });
		...
		try {
			...
			if (typedError.code === 'ER_DUP_ENTRY') {
				return message(reviewForm, STRINGS.PROFILE.MESSAGES.REVIEW_DUPLICATE, {
					status: HTTP_STATUS.CONFLICT
				});
			}
			return message(reviewForm, STRINGS.PROFILE.MESSAGES.REVIEW_FAILED, {

...
const executePayment = async (userId: number, data: z.infer<typeof paymentSchema>) => {
	try {
		const { cartItems, total, shippingAddress, shippingPrice, shippingMethod } = data;
		...
		return jsonResponse(
			...
			STRINGS.CHECKOUT.MESSAGES.PAYMENT_SUCCESS,
			HTTP_STATUS.OK
		);
	} catch (err) {
		logger.error('Payment processing error:', err as Error);
		return errorResponse(
			STRINGS.CHECKOUT.MESSAGES.PAYMENT_FAILED,
...
export const POST: RequestHandler = async ({ request }) => {
	...
	return await executePayment(userId, validation.data);
};

...
const handleLoginError = (error: unknown, correlationId?: string) => {
	if (error instanceof AuthException && error.message === ERROR_CODES.INVALID_CREDENTIALS) {
	...
	logger.error('Database error during login:', error as Error, { correlationId });
	return errorResponse(
...
export const POST: RequestHandler = async ({ request, locals, cookies }) => {
	...
	try {
		...
		return handleLoginError(error, locals?.correlationId);
	}
};

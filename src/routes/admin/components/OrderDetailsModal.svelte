...
>
	<div
		class="w-full max-w-2xl overflow-hidden rounded-2xl bg-surface/95 shadow-2xl backdrop-blur-md"
	...
	>
		<div class="flex items-center justify-between border-b border-secondary/30 px-6 py-4">
			...
			<button
				class="rounded-full p-2 text-secondary transition-colors hover:bg-secondary/20 hover:text-text-main"
		...
		<div class="max-h-[60vh] overflow-y-auto p-6">
			...
			<div class="space-y-4">
				{#each order.items || [] as item}
					<div
						class="flex items-center gap-4 rounded-xl border border-secondary/20 bg-surface-alt/30 p-4"
					>
						<img
							src={item.image}
							alt={item.title}
						...
						<div class="flex-1">
							...
							<p class="text-sm text-text-muted">
								{formatIDR(item.price_at_time)} x {item.quantity}
							</p>
						</div>
			...
			<h3 class="mb-3 mt-8 text-sm font-bold uppercase tracking-wide text-text-muted">
				Shipping Information
			</h3>
			<div
				class="rounded-xl border border-secondary/20 bg-surface-alt/30 p-4 text-sm text-text-main"
			>
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					...
					<div>
						...
						<p class="font-mono font-medium text-primary">
							{order.tracking_number || 'Not updated'}
						</p>
					</div>
					...
					<div class="flex justify-between border-t border-secondary/20 pt-3 sm:col-span-2">

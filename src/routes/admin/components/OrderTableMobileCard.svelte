...
<div class="flex flex-col rounded-xl border border-surface-alt bg-surface-alt/20 p-4 shadow-sm">
	<div class="mb-3 flex flex-col gap-1 border-b border-surface-alt/50 pb-3">
		<div class="flex items-center justify-between">
			...
			<span class="rounded-full px-2.5 py-0.5 text-xs font-semibold {getStatusColor(order.status)}"
				>{order.status}</span
			>
		</div>
	...
	<div class="mb-3 flex justify-between text-sm">
		<div class="flex flex-col">
			...
			<span class="font-semibold text-text-main">
				{order.title === 'Multiple Items' ? '-' : formatIDR(Number(order.price_at_time) || 0)} x {order.quantity ||
					1}
			</span>
		</div>

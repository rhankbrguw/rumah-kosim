...
<ModalShell
	...
	panelClass="flex max-h-[76dvh] flex-col overflow-hidden sm:max-h-[85vh]"
...
>
	<div class="flex shrink-0 border-b border-surface-alt bg-surface-alt/40 px-2">
		{#each [{ id: 'basic', icon: Info, label: STRINGS.ADMIN.MODALS.TABS.BASIC }, { id: 'pricing', icon: Tag, label: STRINGS.ADMIN.MODALS.TABS.PRICING }, { id: 'image', icon: ImageIcon, label: STRINGS.ADMIN.MODALS.TABS.IMAGE }] as tab (tab.id)}
			<button
				class="flex min-h-10 items-center gap-1.5 border-b-2 px-2.5 py-2 text-xs font-medium transition-colors sm:min-h-11 sm:px-4 sm:py-2.5 sm:text-sm {activeTab ===
				tab.id
	...
	<form
		...
		use:enhance
		class="flex-1 overflow-y-auto p-3.5 sm:p-6"
	>
		{#if $message}
			<div
				class="mb-3 rounded-lg bg-primary/10 p-2.5 text-xs text-primary sm:mb-4 sm:p-3 sm:text-sm"
			>
				{$message}
			</div>
		{/if}
		...
		<div
			class="mt-4 flex justify-end gap-2 border-t border-surface-alt pt-3 sm:mt-6 sm:gap-2.5 sm:pt-4"
		>
			<button
				type="button"
				class="min-h-10 cursor-pointer rounded-lg border border-secondary/30 bg-surface px-3 py-2 text-xs font-medium text-text-main transition-colors hover:bg-surface-alt sm:min-h-11 sm:px-4 sm:text-sm"
				on:click={() => dispatch('close')}>{STRINGS.ADMIN.MODALS.ADD_PRODUCT.CANCEL}</button
			>
			<button
				type="submit"
				class="min-h-10 cursor-pointer rounded-lg bg-primary px-3 py-2 text-xs font-medium text-text-inverse shadow-sm transition-colors hover:bg-primary-hover sm:min-h-11 sm:px-4 sm:text-sm"
			>
				{mode === 'add'

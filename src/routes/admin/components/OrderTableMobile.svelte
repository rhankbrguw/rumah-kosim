<script lang="ts">
	...
	import { isOrderLocked, getAvailableStatusTransitions } from '$lib/utils/order.js';
			...
			<div class="flex items-center gap-2 pt-1">
				...
				>
					...
					<div class="relative w-full">
						<select
							name="status"
							class="select-clean w-full cursor-pointer appearance-none rounded-lg border border-secondary/30 bg-surface py-1.5 pl-2.5 pr-7 text-xs font-medium text-text-main shadow-sm transition-colors hover:border-primary focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:cursor-not-allowed disabled:opacity-50"
							bind:value={order.status}
							disabled={isOrderLocked(order.status as string)}
						>
							{#each getAvailableStatusTransitions(order.status as string) as statusOption (statusOption)}

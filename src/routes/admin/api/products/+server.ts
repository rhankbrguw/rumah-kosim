...
export async function GET({ request }) {
	...
	try {
		...
		return errorResponse('Error fetching products', HTTP_STATUS.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR);
	}
}
...
export async function POST({ request }) {
	...
	try {
		...
		if (!validation.success) {
			...
			return errorResponse(firstError, HTTP_STATUS.UNPROCESSABLE_ENTITY, ERROR_CODES.VALIDATION_ERROR);
		}
		...
		try {
			...
			return jsonResponse({ productId: (result as ResultSetHeader).insertId }, 'Product added successfully');
		} catch (dbError) {
			if ((dbError as {code?: string}).code === 'ER_DUP_ENTRY') {
				return errorResponse('A product with this title already exists', HTTP_STATUS.CONFLICT, ERROR_CODES.CONFLICT);
			}
			if ((dbError as {code?: string}).code === 'ER_DATA_TOO_LONG') {
				return errorResponse('One or more fields exceed maximum length', HTTP_STATUS.BAD_REQUEST, ERROR_CODES.VALIDATION_ERROR);
			}
			throw dbError;
		}
	} catch (error) {
		logger.error('POST product error:', error as Error);
		return errorResponse('Error adding product', HTTP_STATUS.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR);
	}
}
...
export async function PATCH({ request }) {
	...
	try {
		...
		if (!productId || !image?.trim()) {
			return errorResponse('Product ID and image are required', HTTP_STATUS.UNPROCESSABLE_ENTITY, ERROR_CODES.VALIDATION_ERROR);
		}
		...
		return errorResponse('Error updating product image', HTTP_STATUS.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR);
	}
}
...
export async function DELETE({ request }) {
	...
	try {
		...
		if (!productId) {
			return errorResponse('Product ID is required', HTTP_STATUS.UNPROCESSABLE_ENTITY, ERROR_CODES.VALIDATION_ERROR);
		}
		...
		return errorResponse('Error deleting product', HTTP_STATUS.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR);
	}
}
